...
class valid_ready_monitor#(
    ...
    `uvm_component_param_utils(valid_ready_monitor#(PARAMS))
    ...
    typedef valid_ready_if#(PARAMS).master intf_t;
...
function void valid_ready_monitor::build_phase(uvm_phase phase);
    ...
    if ( !uvm_config_db#(virtual intf_t)::get(this, "", "vif", vif) )
        `uvm_fatal({get_name(), ".get_fail"}, "Cannot get() virtual interface from uvm_config_db.")
